...
class Cluster:
    ...
    def __call__(self, filename):
        with np.load(self.args['train']) as npz:
        ...
        #train_offsets = pca.fit_transform(train_offsets)
        #print ("Explained variance: ", pca.explained_variance_ratio_.cumsum()[49])
        
        if self.args['k']:
            self.km = KMeans(n_clusters=self.args['k'], n_jobs=-1, random_state=self.RANDOM_SEED)
            self.km.fit_predict(train_offsets)
            #pickle.dump(km, open('kmeans.pickle', 'wb'))
            pickle.dump(self.km, open(filename, 'wb'))
            print('Just written the k-means result for k=%d.' % (self.km.n_clusters))
            #sys.exit(0)
"""
kmeans = {}
